...
//      controllers.add(new MemberController(memberDao));
  ...
  protected void service(HttpServletRequest request, HttpServletResponse response)
    ...
    try {
      // URL 요청을 처리할 rrequest handler를 찾는다
      ...
      // 페이지 컨트롤러에서 오류가 발생했으면 오류 페이지로 포워딩한다
  ...
  private void prepareaPageControllers() throws Exception {
  ...
  private void findComponents(File dir, String packageName) throws Exception {
    ...
    for (File file : files) {
      if (file.isFile()) {
        ...
        Component compoAnno = clazz.getAnnotation(Component.class);
        if (compoAnno != null) {
  ...
  private Object[] prepareRequestHandlerArguments(Method handler, HttpServletRequest request,
    ...
    // 파라밑를 분석하여 각 파라미터에 맞는 값을 배열에 담는다
    for (int i = 0; i < methodParams.length; i++) {
      ...
      if (methodParam.getType() == HttpServletRequest.class
        ...
        if (requestParam != null) {
          ...
          // 그 값을 메소드의 파라미터 타입으로 변환한 후 저장한다
  ...
  private Object createValueObject(Class<?> type, HttpServletRequest request)
    ...
    // 3) 도메인 클래스의 메소드 목록을 가져옴
    Method[] methods = type.getDeclaredMethods();

    // 4) 메소드 중에서 셋터 메소드를 알아냄
    for (Method setter : methods) {
      if (!setter.getName().startsWith("set")) {
        continue;
      }

      // 5) 셋터 메소드의 이름에서 프로퍼티 이름을 추출
