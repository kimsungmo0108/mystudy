...
public class MemberDaoImpl implements MemberDao {
  ...
  public void add(Member member) {
    try {
      ...
      stmt.executeUpdate(
          String.format(
              "insert into membership(email, name, password) values('%s','%s',sha2('%s',256))",
              member.getEmail(),
              member.getName(),
              member.getPassword()));
    } catch (Exception e) {
  ...
  public Member findBy(int no) {
    try {
      Statement stmt = con.createStatement();
      ResultSet rs = stmt.executeQuery("select * from members where no=" + no);
  ...
  public int update(Member member) {
    try {
      ...
      return stmt.executeUpdate(
          String.format(
              "update membership set email='%s', name='%s', password=sha2('%s',256) where no=%d",
              member.getEmail(),
              member.getName(),
              member.getPassword(),
              member.getNo()));
