...
public class Exam0633 {
  ...
  public static void main(String[] args) {
    ...
    // String s1 = "";
    // Predicate<String> p4 = s1::isEmpty;
    // Predicate<String> p4 = (String value) -> {return s1.isEmpty();};
    // String s2 = "hello";
    ...
    // 이유 :
    // 1) isEmpty() 메소드는 스태틱 메소드가 아니기 때문에 스태틱 메소드처럼 호출하면 오류가 발생해야 한다
    // 2) isEmpty() 메소드는 파라미터가 없기 때문에 String 값을 받을 수 없다
    // 그러나 컴파일 오류가 발생하지 않는다

    // 인터페이스의 타입 파라미터에 전달하는 클래스와 메소드 레퍼런스로 전달하는 클래스가 같을 때
    // 위 코드는 다음과 같이 변경된다
